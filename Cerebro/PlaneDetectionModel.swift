...
class PlaneDetectionModel: NSObject {
    ...
    //create ball
    @MainActor func createOrbEntity() -> ModelEntity {
        let orbMesh = MeshResource.generateSphere(radius: 0.05) // 5 cm radius
        var orbMaterial = UnlitMaterial()
        orbMaterial.color = .init(tint: .green.withAlphaComponent(0.8))
        
        let orbEntity = ModelEntity(mesh: orbMesh, materials: [orbMaterial])
        return orbEntity
    }
    
    
    @MainActor func addBall(_ anchor: PlaneAnchor) async throws {
        guard anchor.alignment == .horizontal else { return }
        //only consider tables and seat? where classification found
        ...
        orbEntity.position = SIMD3<Float>(0, 0, 0.1)
        
        //check if entity already exists for this plane, then clear
        if let anchorEntity = rootEntity.findEntity(named: "\(anchor.id)") {
              anchorEntity.children.removeAll()
              anchorEntity.addChild(modelEntity)
              anchorEntity.addChild(textEntity)
              anchorEntity.addChild(orbEntity)
          } else { //add new entity
    ...
    @MainActor func startSession() async {
        ...
        do {
            ...
            for await planeDetectionUpdate in planeDataProvider.anchorUpdates {
                ...
                try await addBall(planeDetectionUpdate.anchor)
